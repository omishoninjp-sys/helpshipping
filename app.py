...
# ============ 維運：指定表內容檢視（唯讀，白名單限定）============
# contact_book 這張表在 repo 全文與 git 全歷史都找不到，不是這套程式建的。
# 欄位特徵（中文欄位名、misc 無型別、mobile_no 宣告 REAL 但 DEFAULT 是字串 'NULL'）
# 指向 Excel/CSV 匯入工具。先看清楚是什麼，再決定修型別還是刪表。
# ★ table 參數走白名單：做成任意表名等於開一個全庫讀取後門。

TABLE_PEEK_ALLOWED = ("contact_book",)
# 判斷「看起來像時間的欄位」：名稱線索
_TIME_NAME_HINTS = ("date", "time", "_at", "created", "updated", "日期", "時間", "建立", "更新")


def _looks_like_time(v):
    """值長得像日期/時間嗎（YYYY-MM-DD 或 YYYY/MM/DD 開頭）。"""
    t = str(v or "").strip()
    return bool(re.match(r"^\d{4}[-/]\d{1,2}[-/]\d{1,2}", t))


@app.route("/api/admin/maintenance/table_peek", methods=["GET"])
def admin_table_peek():
    """看一張表的建表 SQL、欄位、列數與前 10 列（唯讀）。

    ⚠️ 只做 SELECT / PRAGMA，不含任何 UPDATE / INSERT / DELETE / ALTER / DROP / CREATE。
    ⚠️ table 參數走白名單，不接受任意表名。所有值一律 CAST(... AS TEXT) 才輸出。"""
    if not is_boss():
        return jsonify({"success": False, "error": "只有老闆可以執行維運作業"}), 403

    # 不做 strip：只有「完全等於」白名單字串才放行，比對規則越單純越好推理
    table = request.args.get("table") or ""
    if table not in TABLE_PEEK_ALLOWED:
        # 白名單外一律擋下，連查詢都不跑
        return jsonify({"success": False,
                        "error": f"不允許的表名：{table!r}",
                        "allowed": list(TABLE_PEEK_ALLOWED)}), 400
    ...
    try:
        ...
        if not row:
            conn.close()
            return jsonify({"success": False, "error": f"資料庫中沒有 {table} 這張表"}), 404
        create_sql = row["sql"]

        cols = [{"cid": dict(c).get("cid"), "name": dict(c).get("name"),
                 "type": dict(c).get("type") or "(無型別)",
                 "notnull": dict(c).get("notnull"), "dflt_value": dict(c).get("dflt_value"),
                 "pk": dict(c).get("pk")}
                for c in conn.execute(f'PRAGMA table_info("{table}")').fetchall()]
        col_names = [c["name"] for c in cols]
        total = conn.execute(f'SELECT COUNT(*) FROM "{table}"').fetchone()[0]

        sel = ", ".join(f'CAST("{c}" AS TEXT) AS "{c}"' for c in col_names)
        typ = ", ".join(f'typeof("{c}") AS "__t_{c}"' for c in col_names)
        try:
            rows = conn.execute(
                f'SELECT rowid AS __rowid, {sel}, {typ} FROM "{table}" ORDER BY rowid LIMIT 10'
            ).fetchall()
        except Exception:
            rows = conn.execute(f'SELECT {sel}, {typ} FROM "{table}" LIMIT 10').fetchall()

        sample = []
        for r in rows:
            rd = dict(r)
            sample.append({
                "rowid": rd.get("__rowid"),
                "values": {c: rd.get(c) for c in col_names},
                "typeof": {c: rd.get("__t_" + c) for c in col_names},
            })

        # 看起來像時間的欄位 → 給最早/最新，用來判斷這是什麼時候、什麼區間的資料
        time_cols = {}
        for c in col_names:
            by_name = any(h in c.lower() for h in _TIME_NAME_HINTS) or \
                any(h in c for h in ("日期", "時間", "建立", "更新"))
            by_value = any(_looks_like_time((sp["values"] or {}).get(c)) for sp in sample)
            if not (by_name or by_value):
                continue
            mm = conn.execute(
                f'SELECT CAST(MIN("{c}") AS TEXT) AS lo, CAST(MAX("{c}") AS TEXT) AS hi, '
                f'       COUNT("{c}") AS n FROM "{table}" '
                f'WHERE COALESCE("{c}",\'\') <> \'\''
            ).fetchone()
            time_cols[c] = {"earliest": mm["lo"], "latest": mm["hi"], "non_empty": mm["n"],
                            "matched_by": "欄位名" if by_name else "值的格式"}
        conn.close()
        ...
        return jsonify({
            ...
            "row_count": total,
            "sample": {"shown": len(sample), "rows": sample},
...
def admin_last5_diag():
    ...
    2026-09 的 REAL affinity 事件就是靠這支查出來的（見 docs/2026-09-real-affinity-incident.md）：
